...
if __name__ == '__main__':
    station_icao = 'SKBQ'
    (station_lat, station_lon) = os.environ.get('BAQ_STATION_COORDINATES', None).split(',')
    station_coords = [float(station_lat), float(station_lon)]

    gfs_interval_forecast = gfs_interval_forecast = int(os.environ.get('GFS_INTERVAL_FORECAST', 0))
    start_date = datetime.strptime(os.environ.get('START_DATE', None), '%Y-%m-%d %H')
    end_date = datetime.strptime(os.environ.get('END_DATE', None), '%Y-%m-%d %H')
